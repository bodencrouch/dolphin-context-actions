#!/usr/bin/env python3
"""Generate Dolphin service menus from the general conversion registry."""
...
from . import file_converter as converter

MENU_PREFIX = "dolphin-context-actions-convert"
SERVICEMENU_DIR = Path.home() / ".local" / "share" / "kio" / "servicemenus"
CONVERTER_BIN = Path.home() / ".local" / "bin" / "dolphin-context-actions"
    ...
    *,
    ...
    for conv, action_id in zip(featured, action_ids[:-1], strict=True):
        lines.extend(
            [
                ...
                f"Name={conv.label}",
                f"Icon={conv.icon}",
                f'Exec="{converter_bin}" --file-convert {conv.id} %F',
                "",
            ]
        )
    ...
    lines.extend(
        [
            ...
            f'Exec="{converter_bin}" --pick-file-conversion %F',
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to the dolphin-context-actions executable",
    )
    parser.add_argument(
        "--registry",
        type=Path,
